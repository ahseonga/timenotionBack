...
CREATE TABLE TBL_BOARD(
  ...
  CONSTRAINT PK_BOARD PRIMARY KEY (BOARD_ID),
    CONSTRAINT FK_BOARD FOREIGN KEY (USER_ID) REFERENCES TBL_USER(USER_ID)
);
...
CREATE TABLE TBL_FILE(
    ...
    CONSTRAINT FK_FILE FOREIGN KEY (BOARD_ID) REFERENCES TBL_BOARD(BOARD_ID)
);
...
/* UUID
    중복이 가능한 id(식별자)이다
    그러나 중복될 확률이 매우매우 낮기 때문에 중복이 없다고 봐도 무방하다
    사용자가 파일을 업로드할 때 같은 이름의 파일을 올리면 파일끼리 충돌이 발생하므로
   이름 앞에 UUID를 연결하여 중복되는 이름이 없도록 만들어준다
*/
