...
SELECT * FROM GGHJ_BOARD gb WHERE BOARD_LIFE_CYCLE  = '청소년기' AND USER_ID = 1;
-- 유저 소개글 추가해야됨 
=======
...
DROP TABLE GGHJ_KAKAO CASCADE CONSTRAINT;
...
CREATE TABLE GGHJ_USER_FILE(
    ...
    USER_FILE_BACK_NAME      VARCHAR2(255) DEFAULT '' NOT NULL, -- default : 사진 경로
    USER_FILE_BACK_SOURCE    VARCHAR2(255) DEFAULT '' NOT NULL, -- default : 사진 경로
    USER_FILE_PROFILE_UUID   VARCHAR2(255), -- 수정된 부분
    USER_FILE_BACK_UUID      VARCHAR2(255), -- 수정된 부분
    USER_ID                  NUMBER, -- FK
    CONSTRAINT PK_USER_FILE PRIMARY KEY (USER_FILE_ID),
    CONSTRAINT FK_FILE_USER FOREIGN KEY (USER_ID) REFERENCES GGHJ_UNI (UNI_ID)
);
...
DROP TABLE GGHJ_FOLLOW CASCADE CONSTRAINT;
...
DELETE FROM GGHJ_KAKAO gk WHERE kakao_id = 4;
DELETE FROM GGHJ_UNI gu  WHERE uni_id = 9;
